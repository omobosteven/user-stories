...
const StoryComponent = ({ story, onFetchStory, onReviewStory, isAdmin }) => {
	...
	const { summary, description, type, complexity, estimatedHrs, cost, status } = story;
	...
	return (
		<Card title="User Story" className="story-card">
			...
			<div className="btn-group">
				<Button onClick={() => onHandleReview('accepted')} className="btn-accept">
					Accept
				</Button>
				<Button onClick={() => onHandleReview('rejected')} className="btn-reject">
					Reject
				</Button>
			</div>
		</Card>
